...
<div class="col-10 col-sm-12 col-md-10 col-lg-6 mt-5 mx-auto">
  ...
  <div class="d-grid d-md-flex gap-3">
    ...
    <input class="form-check-input" type="radio"  name="availability" id="inStock" value="in_stock"
    {{ $product->availability === 'in_stock' ? 'checked' : '' }}>
     <label class="form-check-label" for="inStock">In Stock</label>
    </div>

    <div class="form-check">
    <input class="form-check-input" type="radio"  name="availability" id="outStock" value="out_of_stock"
    {{ $product->availability === 'out_of_stock' ? 'checked' : '' }}>
...
window.galleryImages.forEach(image => {
    const dz = galleryZones[image.index];
    ...
    let mockFile = {
        name: image.name,
        size: image.size,
        imageId: image.id, // DB ID for removal
        isExisting: true
    };
...
document.querySelector(".btn-addSpec").addEventListener("click", function () {
        ...
        <hr>
            <input name="specification[key][]" class="form-control mb-2 me-1" placeholder="Specification" style="height:65px; font-weight:bold;">
            <textarea name="specification[value][]" class="form-control me-2 mb-2" style="height:65px; font-weight:bold;"></textarea>
            <button type="button" class="btn btn-outline-warning specRemove-btn mb-3" style="height:65px;">(-)</button>
            </div>
...
document.querySelector(".btn-update").addEventListener("click", function(e){
   ...
   if (!mainImageExists) {
    ...
    if (mainFiles.length > 0 && !mainFiles[0].isExisting) {
        formData.append("main_image", mainFiles[0]);
    }
